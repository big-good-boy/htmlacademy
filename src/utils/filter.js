...
export const getArchiveTask = (tasks) => tasks.filter((task) => task.isArchive);

export const getNotArchiveTasks = (tasks) =>
  tasks.filter((task) => !task.isArchive);

export const getFavoriteTask = (tasks) =>
...
export const getRepeatingTasks = (tasks) =>
  tasks.filter((task) => isRepeating(task.repeatingDays));

export const getTasksInOneDay = (tasks, date) =>
  tasks.filter((task) => isOneDay(task.dueDate, date));
...
export const getTasksByFilter = (tasks, filterType) => {
  ...
  switch (filterType) {
    ...
    case FilterType.ARCHIVE:
      return getArchiveTask(tasks);
    case FilterType.FAVORITES:
      return getFavoriteTask(getNotArchiveTasks(tasks));
